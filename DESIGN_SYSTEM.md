...
At widths below 768px, the browser demo fills the viewport and follows the approved 390px mobile composition without a decorative shell. A restrained web-only **Portfolio ↗** return link shares the existing wordmark row without changing native navigation. At tablet and desktop widths, the demo uses a centered 430px app frame on a warm neutral canvas with a subtle border, rounded corners, and shadow; the same return link sits outside that frame so it reads as presentation chrome rather than a product feature. The frame is no taller than 844px, its content scrolls internally, and bottom navigation remains attached to it. Sheets and dialogs are constrained to the same frame on larger screens and remain full-viewport on mobile. Web-native date and time inputs use border-box sizing so their 100% width stays inside sheet padding. The top-level wordmark remains otherwise identical on web and iPhone; no extra prototype label is appended to it.
